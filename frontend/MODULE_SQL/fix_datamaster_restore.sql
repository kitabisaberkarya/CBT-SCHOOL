-- =============================================================================
-- FIX: AUTO-SYNC JURUSAN + ADMIN_RESTORE_DATA
-- CBT School Enterprise
-- Dibuat: 2026-03-06
-- =============================================================================

-- =============================================================================
-- 1. FUNCTION: auto_upsert_majors(major_names text[])
--    Dipanggil dari frontend setelah import siswa untuk sync jurusan
-- =============================================================================
CREATE OR REPLACE FUNCTION public.auto_upsert_majors(major_names text[])
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_major text;
BEGIN
  FOREACH v_major IN ARRAY major_names LOOP
    v_major := TRIM(v_major);
    IF v_major IS NOT NULL AND v_major <> '' AND UPPER(v_major) <> 'STAFF' THEN
...
GRANT EXECUTE ON FUNCTION public.auto_upsert_majors(text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.auto_upsert_majors(text[]) TO anon;

-- =============================================================================
-- 2. TRIGGER: trg_auto_create_major
--    Otomatis insert jurusan ke master_majors saat siswa ditambah/diubah
-- =============================================================================
CREATE OR REPLACE FUNCTION fn_auto_create_major_from_user()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_major TEXT;
BEGIN
  v_major := TRIM(NEW.major);
  IF NEW.role = 'student'
     AND v_major IS NOT NULL
     AND v_major <> ''
     AND UPPER(v_major) <> 'STAFF'
  THEN
    INSERT INTO public.master_majors (name)
    VALUES (v_major)
    ON CONFLICT (name) DO NOTHING;
...
DROP TRIGGER IF EXISTS trg_auto_create_major ON public.users;
CREATE TRIGGER trg_auto_create_major
AFTER INSERT OR UPDATE OF major, role
ON public.users
FOR EACH ROW
EXECUTE FUNCTION fn_auto_create_major_from_user();

-- =============================================================================
-- 3. BACKFILL: Sync jurusan dari siswa yang sudah ada
-- =============================================================================
INSERT INTO public.master_majors (name)
SELECT DISTINCT TRIM(u.major)
FROM public.users u
WHERE u.role = 'student'
  AND TRIM(u.major) IS NOT NULL
  AND TRIM(u.major) <> ''
  AND UPPER(TRIM(u.major)) <> 'STAFF'
ON CONFLICT (name) DO NOTHING;

-- =============================================================================
-- 4. FUNCTION: admin_restore_data(backup_data jsonb)
--    Restore semua data dari file backup JSON aplikasi
-- =============================================================================
...
BEGIN
  ...
  IF backup_data ? 'users' THEN
    FOR user_rec IN SELECT * FROM jsonb_array_elements(backup_data->'users') LOOP
      ...
      IF v_user_id IS NULL THEN
        -- User baru: buat ID baru (jangan pakai ID dari backup karena auth.users)
  ...
  -- ─── 4. TESTS + QUESTIONS ──────────────────────────────────────────────────
  -- Backup format: [[id, {details: {...}, questions: [...]}], ...]
  IF backup_data ? 'tests' THEN
    FOR test_entry IN SELECT * FROM jsonb_array_elements(backup_data->'tests') LOOP
      ...
      -- Hapus soal lama, insert ulang dari backup (preserves ordering)
  ...
  IF backup_data ? 'schedules' THEN
    FOR sched_rec IN SELECT * FROM jsonb_array_elements(backup_data->'schedules') LOOP
      -- Cari test_id berdasarkan testToken
