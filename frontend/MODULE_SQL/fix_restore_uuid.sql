-- =============================================================================
-- FIX: Restore gagal "invalid input syntax for type uuid" (ID legacy base64)
-- CBT School Enterprise
-- Dibuat: 2026-03-06
--
-- Masalah: Data backup lama menyimpan test_id sebagai angka atau base64
-- (contoh: "MTM" = base64("13")), bukan UUID. Saat restore ke sistem baru
-- yang menggunakan UUID, PostgreSQL melempar:
--   ERROR: invalid input syntax for type uuid: "MTM"
--
-- Solusi: Bungkus cast UUID dalam blok EXCEPTION agar jika gagal, sistem
-- mencari test berdasarkan token, atau membuat UUID baru secara otomatis.
-- =============================================================================
...
BEGIN
  ...
  -- ─── 4. TESTS + QUESTIONS ──────────────────────────────────────────────────
  -- PENTING: test_id_raw bisa berupa UUID valid, angka, atau base64 angka (mis "MTM").
  -- Kita gunakan blok EXCEPTION untuk menangani format lama secara aman.
  IF backup_data ? 'tests' THEN
    FOR test_entry IN SELECT * FROM jsonb_array_elements(backup_data->'tests') LOOP
      ...
      -- Coba cast sebagai UUID; jika gagal (format lama), cari by token
      BEGIN
        test_id_val := test_id_raw::uuid;
      EXCEPTION WHEN invalid_text_representation OR OTHERS THEN
        -- ID bukan UUID valid (mis "MTM" = base64("13"), atau angka biasa)
        -- Cari test yang sudah ada berdasarkan token
        SELECT id INTO test_id_val
          FROM public.tests
         WHERE token = test_details->>'token'
         LIMIT 1;
        -- Jika tidak ditemukan, buat UUID baru
        IF test_id_val IS NULL THEN
          test_id_val := gen_random_uuid();
        END IF;
      END;
      ...
      -- Hapus soal lama lalu masukkan ulang
...
-- =============================================================================
-- SELESAI. Script ini sudah dijalankan otomatis di database.
-- Untuk menjalankan ulang: salin dan paste ke Supabase SQL Editor.
-- =============================================================================
