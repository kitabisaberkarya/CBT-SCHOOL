-- =============================================================================
-- FIX: Restore gagal "invalid input syntax for type uuid: HGFDSA"
-- CBT School Enterprise
-- Dibuat: 2026-03-07
--
-- Root cause: Backup dari sistem lain mungkin memiliki ID non-UUID (contoh: "HGFDSA").
-- Cast langsung (...)::uuid akan gagal untuk nilai seperti itu.
-- Fix: Tambah EXCEPTION handler di SETIAP cast ::uuid (master_classes, master_majors,
--       announcements, schedules, dan tests) agar baris yang ID-nya tidak valid UUID
--       tetap diproses menggunakan gen_random_uuid() atau dicari via token/nama.
--
-- Sekaligus mempertahankan fix sebelumnya:
--   #75 extensions.crypt() + extensions.gen_salt('bf')
--   #76 SET statement_timeout = '0'
-- =============================================================================
...
BEGIN
  ...
  IF backup_data ? 'masterData' THEN
    FOR cls_rec IN SELECT * FROM jsonb_array_elements(backup_data->'masterData'->'classes') LOOP
      ...
      EXCEPTION WHEN invalid_text_representation OR OTHERS THEN
        -- ID bukan UUID: cari berdasarkan nama atau buat baru
  ...
  IF backup_data ? 'tests' THEN
    FOR test_entry IN SELECT * FROM jsonb_array_elements(backup_data->'tests') LOOP
      ...
      -- Safe UUID parse untuk test_id
      BEGIN
        test_id_val := test_id_raw::uuid;
      EXCEPTION WHEN invalid_text_representation OR OTHERS THEN
        -- ID bukan UUID: cari berdasarkan token atau buat baru
  ...
  IF backup_data ? 'announcements' THEN
    FOR ann_rec IN SELECT * FROM jsonb_array_elements(backup_data->'announcements') LOOP
      ...
      EXCEPTION WHEN invalid_text_representation OR OTHERS THEN
        -- ID bukan UUID: cari berdasarkan judul atau buat baru
...
-- Fix ini menggabungkan semua perbaikan sebelumnya (#75, #76) + UUID handler baru.
-- =============================================================================
