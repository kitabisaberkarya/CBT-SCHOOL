...
-- Fix: Tambahkan created_by ke admin_restore_data agar soal guru tersimpan saat restore
-- Hanya update bagian INSERT tests dan ON CONFLICT-nya
...
BEGIN
  ...
  IF backup_data ? 'tests' THEN
    FOR test_entry IN SELECT * FROM jsonb_array_elements(backup_data->'tests') LOOP
      ...
      -- Safe UUID parse untuk created_by (soal guru)
      BEGIN
        v_created_by := (test_details->>'created_by')::uuid;
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        kkm                  = EXCLUDED.kkm,
        created_by           = COALESCE(EXCLUDED.created_by, public.tests.created_by),
        updated_at           = NOW();
