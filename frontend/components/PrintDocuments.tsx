      ...
      {documentType === 'minutes' && (
        <>
          <div className="text-[12px] leading-relaxed mb-4 text-justify">
            ...
            <table className="w-full text-[12px] break-inside-avoid"><tbody><tr><td className="text-center align-top" style={{ position: 'relative' }}><p className="mb-1">Mengetahui,</p><p className="mb-1">Kepala Sekolah,</p><div style={{ position: 'relative', display: 'inline-block', minHeight: '80px', minWidth: '140px' }}>{config.signatureUrl ? (<img src={config.signatureUrl} alt="TTD Kepala Sekolah" style={{ height: '64px', display: 'block', margin: '4px auto 0', objectFit: 'contain', position: 'relative', zIndex: 2 }} />) : (<div style={{ height: '64px' }}></div>)}{config.stampUrl && (<img src={config.stampUrl} alt="Stempel" style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-75%, -30%)', width: '110px', height: '110px', objectFit: 'contain', opacity: 0.75, zIndex: 1 }} />)}</div><p className="font-bold uppercase underline underline-offset-2">{config.headmasterName || '....................................'}</p><p>NIP. {config.headmasterNip || '...................'}</p></td></tr></tbody></table>
          </div>
        </>
      )}
