...
const TestModal: React.FC<TestModalProps> = ({ testToEdit, onSave, onClose }) => {
  ...
  const [formData, setFormData] = useState<Omit<TestDetails, 'id' | 'time'>>({
    ...
    kkm: testToEdit?.test.details.kkm ?? 75,
  });
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="subject" value={formData.subject} onChange={handleChange} className="mt-1 w-full p-2 border rounded-md" placeholder="Contoh: Matematika" />
            </div>
            <div>
              ...
              <div className="mt-1 flex items-center gap-1">
                <input
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, kkm: parseFloat(e.target.value) || 0 }))}
                  min="0" max="100" step="0.5"
                  className="w-full p-2 border rounded-md text-center font-bold focus:ring-2 focus:ring-green-500 focus:border-green-500"
                />
                <span className="text-sm text-gray-500 font-medium whitespace-nowrap">/ 100</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Durasi Teks</label>
              <input type="text" name="duration" value={formData.duration} onChange={handleChange} className="mt-1 w-full p-2 border rounded-md" placeholder="90 Menit" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700">Durasi (Menit)</label>
              <input type="number" name="durationMinutes" value={formData.durationMinutes} onChange={handleChange} className="mt-1 w-full p-2 border rounded-md" placeholder="90" />
            </div>
          </div>
          ...
          {/* Opsi Acak */}
          <div className="flex gap-6 p-4 bg-gray-50 border rounded-lg">
            <label className="flex items-center gap-2 cursor-pointer">
              <input id="randomizeQuestions" name="randomizeQuestions" type="checkbox" checked={formData.randomizeQuestions} onChange={handleChange} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
              <span className="text-sm font-bold text-gray-700">Acak Urutan Soal</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input id="randomizeAnswers" name="randomizeAnswers" type="checkbox" checked={formData.randomizeAnswers} onChange={handleChange} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
              <span className="text-sm font-bold text-gray-700">Acak Opsi Jawaban</span>
            </label>
          </div>
          ...
          {!testToEdit && (
            <div>
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="source" value="manual" checked={source === 'manual'} onChange={() => setSource('manual')} className="mr-1" /> Tambah Manual Nanti</label>
                <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="source" value="import" checked={source === 'import'} onChange={() => setSource('import')} className="mr-1" /> Import dari File (.txt)</label>
              </div>
            </div>
          )}
          ...
          <div className="px-6 py-4 border-t bg-gray-50 rounded-b-2xl flex justify-end gap-3 flex-shrink-0">
            ...
            <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-5 rounded-lg transition-colors shadow-sm">Simpan</button>
          </div>
        </form>
      </div>
    </div>
