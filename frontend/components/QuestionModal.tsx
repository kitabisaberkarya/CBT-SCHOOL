...
const QuestionModal: React.FC<QuestionModalProps> = ({ questionToEdit, onSave, onClose }) => {
  ...
  const removeOption = (idx: number) => {
    ...
    setFormData(p => {
      ...
      // Adjust mcKey
      let newMcKey = p.mcKey;
      if (p.mcKey === idx) newMcKey = 0;
      else if (p.mcKey > idx) newMcKey = p.mcKey - 1;
      // Adjust complexMcKeys
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-bold text-gray-800 flex items-center gap-2">
                <span className="bg-green-100 text-green-800 px-2 py-0.5 rounded text-xs">OPSI JAWABAN</span>
            ...
            {formData.options.map((opt, idx) => (
              <div key={idx} className="flex items-start gap-3 p-3 rounded-lg border border-gray-100">
                <div className="pt-3 flex-shrink-0">
                  ...
                  />
                  <div className="text-center font-bold text-gray-400 mt-1 text-sm">{String.fromCharCode(65 + idx)}</div>
                </div>
                ...
                {formData.options.length > 2 && (
                  <button
                    ...
                    className="flex-shrink-0 mt-3 text-red-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-full transition"
            ...
            {formData.options.length < 8 && (
              <button
                type="button"
                onClick={addOption}
                className="w-full py-2.5 border-2 border-dashed border-green-300 hover:border-green-500 text-green-600 hover:text-green-700 font-bold rounded-xl transition-colors flex items-center justify-center gap-2 text-sm"
              >
  ...
  return (
    <div className="fixed inset-0 bg-white z-[70] overflow-y-auto">
      <div className="min-h-full flex flex-col">
        <div className="sticky top-0 px-4 py-3 border-b flex justify-between items-center bg-white z-10 flex-shrink-0">
        ...
        <div className="flex-1 p-3 sm:p-6 space-y-5 sm:space-y-8">
            ...
            <section>
                <label className="block text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-2">Pilih Tipe Soal</label>
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                    {[
                        ...
                        <button
                            ...
                            className={`p-2 sm:p-4 rounded-xl border-2 transition-all text-center flex flex-col items-center justify-center relative ${activeType === type.id ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-100' : 'border-gray-200 hover:border-blue-300 bg-white'}`}
                        >
                            <span className={`font-bold text-xs sm:text-sm ${activeType === type.id ? 'text-blue-600' : 'text-gray-700'}`}>{type.label}</span>
                            <span className={`text-[8px] sm:text-[9px] font-bold tracking-tighter hidden sm:block ${activeType === type.id ? 'text-blue-400' : 'text-gray-400'}`}>{type.sub}</span>
                            {activeType === type.id && <div className="absolute top-1.5 right-1.5 w-2 h-2 bg-blue-500 rounded-full animate-pulse"></div>}
                        </button>
                    ))}
                </div>
            </section>
            ...
            {/* 2. Metadata Bar */}
            <section className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6 p-3 sm:p-5 bg-gray-50 border rounded-2xl">
                <div>
        ...
        <div className="sticky bottom-0 p-3 sm:p-5 border-t flex flex-wrap justify-end gap-2 sm:gap-3 bg-gray-50 flex-shrink-0">
          <button type="button" onClick={onClose} className="px-4 sm:px-6 py-2 sm:py-2.5 bg-white border border-gray-300 hover:bg-gray-100 text-gray-700 font-bold rounded-lg transition-colors text-sm">
            Batal
          </button>

          {!questionToEdit ? (
            <>
                <button onClick={() => handleSubmit(false)} className="px-4 sm:px-6 py-2 sm:py-2.5 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg shadow-md flex items-center transition-all text-sm">
                    ...
                    <span className="hidden sm:inline">Simpan &amp; </span>Tambah Lagi
                </button>
                <button onClick={() => handleSubmit(true)} className="px-4 sm:px-6 py-2 sm:py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-md flex items-center transition-all text-sm">
                    Simpan &amp; Tutup
                </button>
            </>
          ) : (
            <button onClick={() => handleSubmit(true)} className="px-6 sm:px-8 py-2 sm:py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-lg flex items-center transition-all text-sm">
