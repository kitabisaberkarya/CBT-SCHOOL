...
interface DataMasterProps {
  ...
  users: User[];
  onAddItem: (type: ActiveTab, newItemName: string, kkm?: number) => void;
...
const DataMaster: React.FC<DataMasterProps> = ({ masterData, users, onAddItem, onUpdateItem, onDeleteItem, onMergeMasterData }) => {
  ...
  const [newItemKkm, setNewItemKkm] = useState<number>(75);
  ...
  const handleSaveInModal = () => {
    ...
    if (editingItem) { // Update
      onUpdateItem(activeTab, { ...editingItem, name: newItemName.trim(), kkm: activeTab === 'majors' ? newItemKkm : undefined });
    } else { // Add
      onAddItem(activeTab, newItemName.trim(), activeTab === 'majors' ? newItemKkm : undefined);
    }
    ...
    <div className="animate-fade-in flex space-x-6">
      ...
      {isAddEditModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md my-auto border border-gray-100">
            {/* Header */}
            <div className={`px-6 py-4 rounded-t-2xl flex items-center gap-3 ${activeTab === 'majors' ? 'bg-purple-50 border-b border-purple-100' : 'bg-blue-50 border-b border-blue-100'}`}>
              <div className={`p-2 rounded-xl ${activeTab === 'majors' ? 'bg-purple-100' : 'bg-blue-100'}`}>
                <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${activeTab === 'majors' ? 'text-purple-600' : 'text-blue-600'}`} viewBox="0 0 20 20" fill="currentColor">
                  <path d="M10.394 2.08a1 1 0 00-.788 0l-7 3a1 1 0 000 1.84L5.25 8.051a.999.999 0 01.356-.257l4-1.714a1 1 0 11.788 1.838L7.667 9.088l1.94.831a1 1 0 00.787 0l7-3a1 1 0 000-1.838l-7-3z"/>
                </svg>
              </div>
              <h3 className="text-lg font-bold text-gray-800">{editingItem ? 'Edit' : 'Tambah'} {tabTitle}</h3>
            </div>
            {/* Body */}
            <div className="p-6">
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Nama {tabTitle}</label>
              <input
                type="text"
                value={newItemName}
                onChange={(e) => setNewItemName(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleSaveInModal()}
                placeholder={`Masukkan nama ${tabTitle}...`}
            ...
            <div className="px-6 py-4 border-t bg-gray-50 rounded-b-2xl flex justify-end gap-3">
              <button onClick={() => { setIsAddEditModalOpen(false); setNewItemName(''); setNewItemKkm(75); setEditingItem(null); }} className="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-xl text-sm transition-colors">Batal</button>
              <button onClick={handleSaveInModal} className={`px-5 py-2.5 text-white font-bold rounded-xl text-sm shadow-md transition-all ${activeTab === 'majors' ? 'bg-purple-600 hover:bg-purple-700' : 'bg-blue-600 hover:bg-blue-700'}`}>Simpan</button>
            </div>
          </div>
        </div>
      )}
